...
for iter_steps in range(args.states):
    output_dir = os.path.join('output_{}_{}_impg'.format(args.upsample, np.round(0.8**(iter_steps + 1), 4)), args.dataset)
    img_dir = os.path.join(output_dir, 'imgs')
    pth_dir = os.path.join(output_dir, 'pth')
    if not os.path.isdir(img_dir):
        os.makedirs(img_dir)
    if not os.path.isdir(pth_dir):
        os.makedirs(pth_dir)
    #one shot pruning 
    pruning_generate(netG_A2B, args.rate)
    pruning_generate(netG_B2A, args.rate)
    ...
    netD_A.load_state_dict(torch.load(os.path.join(args.rand, 'netD_A_seed_1.pth.tar')))
    netD_B.load_state_dict(torch.load(os.path.join(args.rand, 'netD_B_seed_1.pth.tar')))
    ...
    for epoch in range(args.epoch, args.n_epochs):
        ...
        for i, batch in enumerate(dataloader):
            ...
            # print loss:
            if i % 100 == 0:
                print('State: %d, epoch %d-%d: loss_G %.4f, loss_D %.4f' % (
                    iter_steps, epoch, i, loss_G.data, (loss_D_A + loss_D_B).data))
