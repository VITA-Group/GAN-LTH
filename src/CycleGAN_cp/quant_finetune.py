'''
Use GS32 to train a 32 bit model. 
Then use GS8 to finetune the 32 bit initialized model to 8 bit (both weight and activations).
'''
...
# the subnetwork should be defined as 8 bit model:
quant = True
# the model used for initialization should be 32 bit:
assert 'GS32' in args.base_model_str 
assert 'GS8' not in args.base_model_str 
...
loss_str = ''
results_dir = os.path.join('quant_finetune_results', args.dataset, args.task, args.base_model_str, 
...
for epoch in range(start_epoch, args.epochs):
    ...
    if epoch > int(args.epochs/2) and epoch % 10 == 0 or epoch == args.epochs - 1:    
        ...
        if FID < best_FID:
            ...
            # f_best_FID_epoch = open(os.path.join(results_dir, 'best_FID_epoch.txt'), 'a')
            # f_best_FID_epoch.write('%d' % (best_FID_epoch))
            # f_best_FID_epoch.close()
