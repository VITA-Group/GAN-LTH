...
def main():
    ...
    start_epoch = 0
    best_fid = 1e4
    args.path_helper = set_log_dir('logs', args.exp_name + "_{}_imp".format(0.8))
    ...
    rewind_checkpoint_file = os.path.join(args.rewind_path, 'Model', 'checkpoint.pth')
    assert os.path.exists(checkpoint_file)
    checkpoint = torch.load(checkpoint_file)
    rewind_checkpoint = torch.load(rewind_checkpoint_file)
    #gen_net.load_state_dict(checkpoint['gen_state_dict'])
    # Not need this because we need initial weight

    gen_net.load_state_dict(checkpoint['gen_state_dict'])
    dis_net.load_state_dict(checkpoint['dis_state_dict'])
    round_ = 0
    print("=> Loaded rewind checkpoint: {}".format(rewind_checkpoint_file))
    for ro in range(10):
        best_fid = 1e4
        ...
        pruning_generate(gen_net, 0.2)
        pruning_generate(avg_gen_net, 0.2)
        see_remain_rate(gen_net)

        
        dis_net.load_state_dict(rewind_checkpoint['dis_state_dict']) 
               
        gen_weight = gen_net.state_dict()
        gen_orig_weight = rewind_weight(rewind_checkpoint['gen_state_dict'], gen_weight.keys())
        start_epoch = rewind_checkpoint['epoch']
        gen_weight.update(gen_orig_weight)
        gen_net.load_state_dict(gen_weight)
        
        #gen_optimizer.load_state_dict(rewind_checkpoint['gen_optimizer'])
        #dis_optimizer.load_state_dict(rewind_checkpoint['dis_optimizer'])
